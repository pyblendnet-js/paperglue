...
function init() {
	...
	nodeComms = window.globals.nodeComms;
	paperGlue.appendMenus({file:fileMenu,defaultContextMenu:defaultMenuAddendum});
  onKeyChain = window.globals.chainKey;
  window.globals.chainKey = onKey;
}
...
function fileSelectorDialog(objective, xtns, dir_obj) {
  ...
  fileSelectObjective = objective;
	dialog.fileSelector(objective, xtns, dir_obj, {
		module:"paperGlueDialogs",funct:objective},fileSelected
  );
}

function fileSelected(path,subpath) {
  console.log("FileSelected:"+path+" / "+subpath);
      ...
      if(path === "")
        path = "localImages";
			dialog.fileSelector("loadImage", "localImages", {
				...
				path: path,
				dir: ilist
			},{module:"pgdialogs",funct:loadImage},loadImage);  // returns to self
//*************** wrong args for loadImage as fileSelector!
    ...
    nodeComms.setFileSelector(fileSelectorDialog);
		...
		paperGlue.loadSingleImage(full_path, subpath);
	}
}
