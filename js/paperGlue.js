...
(function() { // using the module pattern to provide name space
	...
	*/
	function loadCurrentContextMenu(tbl) {
		...
		for (var mi in currentContextMenu) {
			...
			tbl.innerHTML += '<tr><td style="padding:0;"><div style="font-size:' +
				fontsize + 'px;" class="ContextItem" onmouseup="contextMenuCallback(' +
				mi +
				')" >' + txt + '</div></td></tr>';
		}
	}
	...
	function selectLine(event) {
		...
		var line_select_fraction = (event.point.subtract(lineSelected.segments[0].point))
			.length /
			lineSelected.length;
			...
			if (selectedMove) {
				for (var sid in selectedItems) {
					...
					if (lineInstances.hasOwnProperty(sid))
						lineInstances[sid].path.position = lineInstances[sid].path.position.add(
							event.delta);
					else if (symbolInstances.hasOwnProperty(sid))
						symbolInstances[sid].raster.position = symbolInstances[sid].raster.position
						.add(event.delta);
			...
			if (Math.abs(v.x) > minAreaSide && Math.abs(v.y) > minAreaSide) {
				...
				switch (areaSelectMode) { //chosen in onlinemousedown
					case 0: //move last point
						...
						areaSelected.segments[2].point = areaSelected.segments[2].point.add(
							event.delta);
				...
				switch (lineSelectMode) { //chosen in onlinemousedown
					case 0: //move last point
						if (lineSelected.segments.length < 2) {
							...
							lineSelected.lastSegment.point = lineSelected.lastSegment.point.add(
								event.delta);
						}
						break;
					case 1: //move first point
						lineSelected.firstSegment.point = lineSelected.firstSegment.point.add(
							event.delta);
						break;
					default: // move all
	...
	function onMouseUp(event) {
		...
		if (editMode) {
			...
			if (!!lineSelected) {
				...
				hideCursor();
				imageMoved(imageSelected, imageSelectedPosition, (imageSelected.position.equals(
	...
	function correctPosition(raster, src, round_only) {
		...
		if (src.hasOwnProperty('origin')) {
			...
			raster.position = snapPoint(raster.position.subtract(oo), round_only).add(
				oo);
		} else {
	...
	function rotateImage(id, type, raster, src, angle) {
		...
		if (src.hasOwnProperty('center')) {
			...
			raster.rotate(angle, raster.position.subtract(src.center.rotate(raster.rotation))); //(new Point(30,30)));  // need to look up source to find centre of rotation - could also have 45 deg mode
		} else
	...
	function moveCurrentImage(x, y, r) {
		...
		if (r !== im.rotation) {
			rotateImage(currentContextObject.id, currentContextObject.type, im,
	...
	function validatePath(path, force_id) {
		...
		if (snapLine(path, round_only)) {
			...
			if (typeof force_id == 'undefined') { // else don't record redos
				var np = [lineSelected.firstSegment.point.clone(), lineSelected.lastSegment
					.point.clone()
				];
				var do_rec = {
					action: 'lineMove',
	...
	function removeLine(id, record) {
		if (lineInstances.hasOwnProperty(id)) { // this path does exist
			...
			if (record) {
				var lp = [line.path.firstSegment.point.clone(), line.path.lastSegment.point
					.clone()
				];
	...
	function onKeyDown(event) { //note: this is the paper.js handler - do not confuse with html
		...
		event.controlPressed = ((nowMs - controlPressMs) < modPressDelay) ||
			event.modifiers
			.control;
		event.shiftPressed = ((nowMs - shiftPressMs) < modPressDelay) || event.modifiers
			.shift;
		...
		if (event.controlPressed) {
			...
			if (!!delta) {
				...
				var moveby = [Math.round(delta[0] * grad.width), Math.round(delta[1] *
					grad
					.height)];
	...
	function incMoveSymbol(obj, direction, snap) {
		...
		if (snap) {
			obj.raster.position = obj.raster.position.add([direction[0] * snapRect[2],
	...
	function listFiles(objective, xtns, path, subpath) {
		console.log("List " + xtns + " from " + path + " / " + subpath + "for " +
			objective);
		...
		switch (last_do.action) {
			...
			case 'imageRotate':
				...
				if (typeof last_do.oldValue != 'undefined') { // check it has a pos
					...
					console.log("Return img rotation to " + raster.rotation + "at pos " +
						raster.position);
				}
				break;
			case 'lineDelete':
	...
	function areaSelect() {
		...
		for (var id in lineInstances) {
			var l = lineInstances[id];
			if (rect.contains(l.path.firstSegment.point) && rect.contains(l.path.lastSegment
					.point)) {
	...
	*/
	...
	// following replaced by new dependancy checks
